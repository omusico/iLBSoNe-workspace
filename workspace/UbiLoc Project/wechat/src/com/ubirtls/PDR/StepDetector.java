...
public class StepDetector {
	...
	int w = 2;
	/**保存数据用于下次计算*/
	private double[] smoothWindows = new double[sampleMaxCount - sampleMinCount + w];
	...
	public StepDetectResult detectStep(List<double[]> accS) {
		...
		/*		if(m+2 * sampleMinCount > accSize)
			Log.i("accErrorErrorrrrrrrrrrrrr", String.valueOf(accSize));
		 */
		if (m + 2 * sampleMinCount <= accSize) {
			...
			for (int sampleCountOpt = sampleMinCount; sampleCountOpt <= sampleMaxCount; sampleCountOpt++) {
				if(m+2*sampleCountOpt <= accSize){
					double cofTemp = correlation(MyMath.cloneSubList(localMeanAcceleration,m, m+2*sampleCountOpt-1));
					if (cofTemp > cof){
						cof = cofTemp;
			...
			double stdAcc = MyMath.std(MyMath.cloneSubList(localMeanAcceleration, m, m+2*sampleCountTemp-1));
			if(stdAcc <= stdThreshold){
				...
				/*计算第一步步长*/
				double stepOneLength = CalculateStrideLength(
						MyMath.max(MyMath.cloneSubList(localMeanAcceleration,stepOneStPoint,stepOneEndPoint)),
						MyMath.min(MyMath.cloneSubList(localMeanAcceleration,stepOneStPoint,stepOneEndPoint)));

				/*计算第二步步长*/
				double stepTwoLength = CalculateStrideLength(
						MyMath.max(MyMath.cloneSubList(localMeanAcceleration,stepTwoStPoint,stepTwoEndPoint)),
						MyMath.min(MyMath.cloneSubList(localMeanAcceleration,stepTwoStPoint,stepTwoEndPoint)));
				result = new StepDetectResult(true,stepOneLength,stepTwoLength,deletedSampleCount);
			}
			...
			preWindows[1] = smoothWindows[sampleCountTemp - this.sampleMinCount+1];
	...
	private List<Double> highPassFilter(List<Double> magnitudeOfAcceleration) {
		...
		for (int i = 0; i < magnitudeOfAcceleration.size(); i++) {
			...
			if(isFirst)
				first = 1;
			if(i>= (this.sampleMinCount-1+ first) && (i<= this.sampleMaxCount-1+ first))
