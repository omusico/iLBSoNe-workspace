...
public class MapDatabase {
	...
	private static final Logger LOG = Logger.getLogger(MapDatabase.class
			.getName());
	...
	/**
	 * Closes the map file and destroys all internal caches. This method has no
	 * effect if no map file is currently opened.
	 */
	...
	public void executeQuery(Tile tile, MapDatabaseCallback mapDatabaseCallback) {
		try {
			...
			queryParameters.queryZoomLevel = this.mapFileHeader
			...
			QueryCalculations.calculateBaseTiles(queryParameters, tile,
					subFileParameter);
			QueryCalculations
					.calculateBlocks(queryParameters, subFileParameter);

			processBlocks(mapDatabaseCallback, queryParameters,
					subFileParameter);
		} catch (IOException e) {
			LOG.log(Level.SEVERE, null, e);
		}
	...
	public FileOpenResult openFile(File mapFile) {
		try {
			...
			if (this.inputFile != null) {
				...
				FileOpenResult fileOpenResult = this.mapFileHeader.readHeader(
						this.readBuffer, this.fileSize);
				if (!fileOpenResult.isSuccess()) {
					closeFile();
					return fileOpenResult;
				}
			}else{
				Log.e("abc", "kao kao error");
			}
	...
	private void decodeWayNodesDoubleDelta(float[] waySegment) {
		...
		int wayNodeLatitude = this.tileLatitude
				+ this.readBuffer.readSignedInt();

		// get the first way node longitude offset (VBE-S)
		int wayNodeLongitude = this.tileLongitude
				+ this.readBuffer.readSignedInt();
		...
		for (int wayNodesIndex = 2; wayNodesIndex < waySegment.length; wayNodesIndex += 2) {
			...
			int singleDeltaLatitude = doubleDeltaLatitude
					+ previousSingleDeltaLatitude;
			int singleDeltaLongitude = doubleDeltaLongitude
					+ previousSingleDeltaLongitude;
	...
	private void processBlock(QueryParameters queryParameters,
		...
		int zoomTableRow = queryParameters.queryZoomLevel
				- subFileParameter.zoomLevelMin;
		...
		if (this.readBuffer.getBufferPosition() > firstWayOffset) {
			LOG.warning("invalid buffer position: "
					+ this.readBuffer.getBufferPosition());
			if (this.mapFileHeader.getMapFileInfo().debugFile) {
				LOG.warning(DEBUG_SIGNATURE_BLOCK + this.signatureBlock);
			}
			return;
		...
		if (!processWays(queryParameters, mapDatabaseCallback,
				waysOnQueryZoomLevel)) {
			return;
		}
	}

	private void processBlocks(MapDatabaseCallback mapDatabaseCallback,
			QueryParameters queryParameters, SubFileParameter subFileParameter)
			throws IOException {
		...
		for (long row = queryParameters.fromBlockY; row <= queryParameters.toBlockY; ++row) {
			for (long column = queryParameters.fromBlockX; column <= queryParameters.toBlockX; ++column) {
				...
				long currentBlockIndexEntry = this.databaseIndexCache
				...
				if (blockNumber + 1 == subFileParameter.numberOfBlocks) {
					...
					if (nextBlockPointer < 1
							|| nextBlockPointer > subFileParameter.subFileSize) {
						LOG.warning("invalid next block pointer: "
								+ nextBlockPointer);
						LOG.warning("sub-file size: "
				...
				if (currentBlockSize < 0) {
					...
					// block
					LOG.warning("current block size too large: "
							+ currentBlockSize);
					continue;
				...
				this.inputFile.seek(subFileParameter.startAddress
						+ currentBlockPointer);

				// read the current block into the buffer
				if (!this.readBuffer.readFromFile(currentBlockSize)) {
					// skip the current block
					LOG.warning("reading current block has failed: "
							+ currentBlockSize);
					return;
				}
				...
				try {
					processBlock(queryParameters, subFileParameter,
							mapDatabaseCallback);
	...
	private boolean processBlockSignature() {
		if (this.mapFileHeader.getMapFileInfo().debugFile) {
			...
			this.signatureBlock = this.readBuffer
					.readUTF8EncodedString(SIGNATURE_LENGTH_BLOCK);
	 ...
	 *         otherwise.
	 */
	private boolean processPOIs(MapDatabaseCallback mapDatabaseCallback,
			int numberOfPois) {
		...
		for (int elementCounter = numberOfPois; elementCounter != 0; --elementCounter) {
			if (this.mapFileHeader.getMapFileInfo().debugFile) {
				...
				this.signaturePoi = this.readBuffer
						.readUTF8EncodedString(SIGNATURE_LENGTH_POI);
			...
			int longitude = this.tileLongitude
					+ this.readBuffer.readSignedInt();
			...
			if (featureName) {
				tags.add(new Tag(TAG_KEY_NAME, this.readBuffer
						.readUTF8EncodedString()));
			}

			// check if the POI has a house number
			if (featureHouseNumber) {
				tags.add(new Tag(TAG_KEY_HOUSE_NUMBER, this.readBuffer
						.readUTF8EncodedString()));
			}

			// check if the POI has an elevation
			if (featureElevation) {
				tags.add(new Tag(TAG_KEY_ELE, Integer.toString(this.readBuffer
						.readSignedInt())));
			}

			mapDatabaseCallback.renderPointOfInterest(layer, latitude,
					longitude, tags);
		}
	...
	private float[][] processWayDataBlock(boolean doubleDeltaEncoding) {
		...
		if (numberOfWayCoordinateBlocks < 1
				|| numberOfWayCoordinateBlocks > Short.MAX_VALUE) {
			LOG.warning("invalid number of way coordinate blocks: "
					+ numberOfWayCoordinateBlocks);
			logDebugSignatures();
			return null;
		}
		...
		for (int coordinateBlock = 0; coordinateBlock < numberOfWayCoordinateBlocks; ++coordinateBlock) {
			...
			if (numberOfWayNodes < 2
					|| numberOfWayNodes > MAXIMUM_WAY_NODES_SEQUENCE_LENGTH) {
	...
	private boolean processWays(QueryParameters queryParameters,
			MapDatabaseCallback mapDatabaseCallback, int numberOfWays) {
		List<Tag> tags = new ArrayList<Tag>();
		...
		for (int elementCounter = numberOfWays; elementCounter != 0; --elementCounter) {
			if (this.mapFileHeader.getMapFileInfo().debugFile) {
				...
				this.signatureWay = this.readBuffer
						.readUTF8EncodedString(SIGNATURE_LENGTH_WAY);
			...
			if (featureRef) {
				tags.add(new Tag(TAG_KEY_REF, this.readBuffer
						.readUTF8EncodedString()));
			}
			...
			if (wayDataBlocks < 1) {
				LOG.warning("invalid number of way data blocks: "
						+ wayDataBlocks);
				logDebugSignatures();
				return false;
			}
			...
			for (int wayDataBlock = 0; wayDataBlock < wayDataBlocks; ++wayDataBlock) {
				...
				mapDatabaseCallback.renderWay(layer, labelPosition, tags,
						wayNodes);
			}
		}
	...
	private float[] readOptionalLabelPosition(boolean featureLabelPosition) {
		...
		if (featureLabelPosition) {
			...
			labelPosition[1] = this.tileLatitude
					+ this.readBuffer.readSignedInt();

			// get the label position longitude offset (VBE-S)
			labelPosition[0] = this.tileLongitude
					+ this.readBuffer.readSignedInt();
		}
	...
	private int[][] readZoomTable(SubFileParameter subFileParameter) {
		int rows = subFileParameter.zoomLevelMax
				- subFileParameter.zoomLevelMin + 1;
