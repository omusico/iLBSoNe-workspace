 ...
 */
package eu.geopaparazzi.spatialite.database.spatial.core.sqlitedb;
...
public class SqlitedbDroidSpitter {
    ...
    /**
      ...
      */
    public SqlitedbDroidSpitter( File file_mbtiles, HashMap<String, String> mbtiles_metadata ) {
    ...
    public byte[] getTileAsBytes( int i_x, int i_y_osm, int i_z ) {
        ...
        try {
            ...
            c = db_mbtiles.rawQuery("select  image as tile_data from tiles where x=? and y=? and z=?",
                    new String[]{s_x, s_y, s_z});
            ...
            c.close(); // db_lock.readLock().unlock();
        }
        return blob_data;
    }
    // -----------------------------------------------
    ...
    private int insertTile( String s_tile_id, int i_x, int i_y_osm, int i_z, byte[] ba_tile_data, int i_force_unique )
        ...
        int i_y = i_y_osm;
        if (s_tile_row_type.equals("tms")) {
            int[] tmsTileXY = SqlitedbDroidSpitter.googleTile2TmsTile(i_x, i_y_osm, i_z);
            i_y = tmsTileXY[1];
        }
        if (i_force_unique > 1)
            i_force_unique = 0;
    ...
    /**
      ...
      */
    public SqlitedbMetadata fetchMetadata( String metadataVersion ) throws MetadataParseException {
        /*Cursor c = db_mbtiles.query(SqlitedbSQLite.TABLE_METADATA, new String[]{SqlitedbSQLite.COL_METADATA_NAME,
                SqlitedbSQLite.COL_METADATA_VALUE}, null, null, null, null, null);
        MetadataValidator validator = SqlitedbMetadata.MetadataValidatorFactory.getMetadataValidatorFromVersion(metadataVersion);
        if (validator == null)
            return null;
        this.metadata = SqlitedbMetadata.createFromCursor(c, c.getColumnIndex(SqlitedbSQLite.COL_METADATA_NAME),
                c.getColumnIndex(SqlitedbSQLite.COL_METADATA_VALUE), validator);
    ...
    public List<String> build_request_list( double[] request_bounds, int i_zoom_level, String s_request_type,
        ...
        if (s_tile_row_type.equals("tms")) {
            int[] tmsTileXY = SqlitedbDroidSpitter.googleTile2TmsTile(i_min_x, i_min_y_osm, i_zoom_level);
            i_min_y_tms = tmsTileXY[1];
            i_min_y = i_min_y_tms;
            tmsTileXY = SqlitedbDroidSpitter.googleTile2TmsTile(i_max_x, i_max_y_osm, i_zoom_level);
    ...
    /**
      ...
      * @return HashMap<String,String> metadate [key,value]
      */
    public SqlitedbMetadata getMetadata() {
        return this.metadata;
    }
    // -----------------------------------------------
    /**
