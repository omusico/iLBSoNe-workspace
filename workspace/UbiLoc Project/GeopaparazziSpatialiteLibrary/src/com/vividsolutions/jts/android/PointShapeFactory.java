...
public interface PointShapeFactory {
    ...
    public static class POP extends BasePointShapeFactory {
        /**
         * Creates a new factory for points with default size.
         * 
         */
         ...
         */
        public DrawableShape createPoint( PointF point ) {
            return new PathShape(createballoonPath(point, (float) size));
        }
        public Path createballoonPath( PointF point, float size ) {

            Path path = new Path();
            // 画上边的圆弧
            RectF oval = new RectF(point.x - size, point.y - size, point.x + size, point.y + size);
            path.addArc(oval, 135, 270);

            // 画右下的曲线
            float startXR = (float) Math.sqrt(2) * size / 2 + point.x;
            float startYR = (float) Math.sqrt(2) * size / 2 + point.y;
            float bezierXR = point.x;
            float bezierYR = (float) Math.sqrt(2) * size + point.y;
            float endXR = point.x;
            float endYR = (float) Math.sqrt(3) * size + point.y;
            path.quadTo(startXR, startYR, (startXR + bezierXR) / 2, (startYR + bezierYR) / 2);
            path.quadTo(bezierXR, bezierYR, (endXR + bezierXR) / 2, (endYR + bezierYR) / 2);

            // 画左下的曲线
            float startXL = (float) -Math.sqrt(2) * size / 2 + point.x;
            float startYL = (float) Math.sqrt(2) * size / 2 + point.y;
            float bezierXL = point.x;
            float bezierYL = (float) Math.sqrt(2) * size + point.y;
            float endXL = point.x;
            float endYL = (float) Math.sqrt(3) * size + point.y;
            path.moveTo(startXL, startYL);
            path.quadTo(startXL, startYL, (startXL + bezierXL) / 2, (startYL + bezierYL) / 2);
            path.quadTo(bezierXL, bezierYL, (endXL + bezierXL) / 2, (endYL + bezierYL) / 2);
            path.close();
            return path;
