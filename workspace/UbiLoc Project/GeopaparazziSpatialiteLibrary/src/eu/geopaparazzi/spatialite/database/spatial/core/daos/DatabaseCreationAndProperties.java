...
public class DatabaseCreationAndProperties implements ISpatialiteTableAndFieldsNames {
    ...
    public static SpatialiteDatabaseType checkDatabaseTypeAndValidity( Database database,
            HashMap<String, String> spatialVectorMap, HashMap<String, String> spatialVectorMapErrors ) throws Exception {
        // clear views
        spatialVectorMap.clear();
        spatialVectorMapErrors.clear();
        if (DatabaseCreationAndProperties.JavaSqliteDescription.equals("")) { // Rasterlite2Version_CPU
                                                                              // will NOT be empty,
                                                                              // if the
            ...
            GPLog.addLogEntry("DAOSPATIALIE", "JavaSqliteDescription[" + DatabaseCreationAndProperties.JavaSqliteDescription
                    + "] recovery_mode[" + SPL_Vectors.VECTORLAYER_QUERYMODE + "]");
        }
        ...
        if (table.equals("")) {
            if ((b_geometry_columns) && (b_spatial_ref_sys)) {
                if (b_spatial_ref_sys) {
                    ...
                    if (versionFromSrswktPresence == SpatialiteVersion.AFTER_4_0_0_RC1) { // Spatialite
                                                                                          // 4.0
            ...
            if (b_spatial_ref_sys) { // 'proj4text' must always exist - otherwise invalid
                ...
                case SRS_WKT__NOTFOUND_PRE_2_4_0:
                    spatialiteVersion = SpatialiteVersion.UNTIL_2_4_0; // no 'srs_wkt' or 'srtext'
                                                                       // fields
                    break;
