...
public class SPL_Vectors implements ISpatialiteTableAndFieldsNames {
        ...
        try {
            ...
            if (statement.step()) {
                ...
                if (spatialIndex == 1) {
                    ...
                    if (fieldNamesToTypeMap.size() > 0) { // AbstractSpatialTable
    ...
    private static String getSpatialiteUpdateLayerStatistics( Database database, String tableName, String geometryColumn,
        ...
        if (DaoSpatialite.getGeometriesCount(database, tableName, geometryColumn) == 0) {
            // GPLog.addLogEntry(LOGTAG, "getSpatialiteUpdateLayerStatistics[" + databaseType +
            // "] error[getGeometriesCount == 0] db[" + database.getFilename() + "]");
            return vectorExtent;
        }
        if (spatialIndex == 1) {
            spatialIndex = spatialiteUpdateLayerStatistics(database, tableName, geometryColumn, spatialIndex, databaseType);
            if (spatialIndex != 1) {
                // GPLog.addLogEntry(LOGTAG, "getSpatialiteUpdateLayerStatistics[" + databaseType +
                // "] error[UpdateLayerStatistic != 1 ][" + spatialIndex + "] db[" +
                // database.getFilename() + "]");
                return vectorExtent; // Invalid for use with geopaparazzi
            }
        }
        // for table/geometry support, otherwise for whole Database (Spatialite3+4) try to retrieve
        // the needed bounds again
        if ((!tableName.equals("")) && (!geometryColumn.equals(""))) {
            ...
            if (vectorExtent.equals("")) {
                ...
                if (vectorExtent.equals("")) {
                    // GPLog.addLogEntry(LOGTAG, "getSpatialiteUpdateLayerStatistics[" +
                    // databaseType + "] error[GeometriesBoundsString empty] db[" +
                    // database.getFilename() + "]");
                }
            }
        }
            ...
            for( Map.Entry<String, String> vector_entry : spatialVectorMapErrors.entrySet() ) {
                ...
                if (sa_string.length == 5) {
                    ...
                    if (sa_string.length == 7) { // vector_value[1;2;2913;1;row_count;extent_min_x,extent_min_y,extent_max_x,extent_max_y;last_verified]
                        ...
                        // always be 1
                        // This should NOT be the default behavior, there may be a reason why no
                        // SpatialIndex was created
                        if ((i_spatial_index_enabled == 0)
                                && (VECTORLAYER_QUERYMODE == VectorLayerQueryModes.CORRECTIVE || VECTORLAYER_QUERYMODE == VectorLayerQueryModes.CORRECTIVEWITHINDEX)) {
                            i_spatial_index_enabled = SpatialiteIndexing.spatialiteCreateSpatialIndex(database, table_name,
                                    geometry_column, databaseType);
                        ...
                        // if
                        // (s_bounds.equals("extent_min_x,extent_min_y,extent_max_x,extent_max_y"))
                        // {
                        // /*
                        // * if i_row_count == 0 :
                        ...
                        if (i_row_count != 0) {
                            ...
                            if (VECTORLAYER_QUERYMODE != VectorLayerQueryModes.STRICT
                                    && VECTORLAYER_QUERYMODE != VectorLayerQueryModes.TOLERANT) {
                        ...
                        if (!vector_extent.equals("")) { // all of the geomtries of this column may
                            ...
                            if (VECTORLAYER_QUERYMODE != VectorLayerQueryModes.STRICT
                                    && VECTORLAYER_QUERYMODE != VectorLayerQueryModes.TOLERANT) { // remove
                                                                                                  // from
                                                                                                  // the
                                                                                                  // errors,
                                                                                                  // since
    ...
    static void getSpatialVectorMap_V4( Database database, HashMap<String, String> spatialVectorMap,
        ...
        // if empty: there are nothing to correct [do before RasterLite2 logic - there is no error
        // control for that]
        if ((VECTORLAYER_QUERYMODE != VectorLayerQueryModes.STRICT) && (spatialVectorMapErrors.size() > 0)) {
