...
public class HeadingKalmanFilter {
	 ...
	 * @param kg 卡尔曼增益
	 */
	public HeadingKalmanFilter(double q,double r,double pre_p,double pre_ori, double gyroHZ){
		Q = q;
		R = r;
		this.pre_p = pre_p;
	...
	public HeadingKalmanFilter(double pre_ori){
		double temp = (90-(pre_ori-10))%360;//(90-(360 + 260 - pre_ori))%360;
		if(temp<0)
	...
	/**启动卡尔曼滤波*/
	public double getFilteredHeading(ArrayList<Double> oriX, ArrayList<Double> gyroZ){
		int oriSize = oriX.size();
		int gyroSize = gyroZ.size();
		/**对方向数据进行处理,电子罗盘获得方位需进行校准（三星手机）*/
		for (int i = 0; i< oriSize; i++)
		{
			double temp = (90-(oriX.get(i)-10))%360; // (90-(360 + 260 - oriX.get(i)))%360;
			if(temp<0)
				oriX.set(i ,360 + temp);
			else 
				oriX.set(i , temp);
		...
		for (int k = 1; k < oriSize; k++){
			...
			/**状态预测*/
			double dQ = MyMath.sum(MyMath.cloneSubList(gyroZ,gyroStart, gyroEnd))*gyroHZ*180/PI;
			cur_ori = pre_ori + dQ ;
			cur_p = pre_p + Q;
			kg = cur_p/(cur_p + R);
			if (Math.abs(oriX.get(k)-cur_ori)>180){
				if (oriX.get(k) < cur_ori)
					cur_ori = mod(cur_ori + kg*(oriX.get(k)+360-cur_ori),360);
				else
					cur_ori = mod(cur_ori + kg*(oriX.get(k)-cur_ori-360),360);
			}
			else
