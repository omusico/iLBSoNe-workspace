...
import eu.geopaparazzi.spatialite.database.spatial.core.sqlitedb.SqlitedbAsync;
import eu.geopaparazzi.spatialite.database.spatial.core.sqlitedb.SqlitedbDroidSpitter;
import eu.geopaparazzi.spatialite.database.spatial.core.sqlitedb.SqlitedbMetadata;
...
public class SqlitedbDatabaseHandler extends AbstractSpatialDatabaseHandler {

    private List<SpatialRasterTable> rasterTableList;
    private SqlitedbDroidSpitter mbtilesSplitter;
    ...
    private SqlitedbAsync mbtiles_async = null;
     ...
     */
    public List<SqlitedbDatabaseHandler.AsyncTasks> asyncTasksList = new ArrayList<SqlitedbDatabaseHandler.AsyncTasks>();
     ...
     * @throws IOException  if something goes wrong. 
      */
    public SqlitedbDatabaseHandler( String dbPath, DataBaseBean databaseBean ) throws IOException {
        super(dbPathCheck(dbPath));
        this.databaseBean = databaseBean;
        mbtilesSplitter = new SqlitedbDroidSpitter(databaseFile, mbtilesMetadata);
    }
    ...
    private static String dbPathCheck( String dbPath ) {
        if (!dbPath.endsWith(SpatialDataType.SQLITEDB.getExtension())) {
            // .mbtiles files must have an .mbtiles
            // extension, force this
            dbPath = dbPath.substring(0, dbPath.lastIndexOf(".")) + SpatialDataType.SQLITEDB.getExtension();
        }
        return dbPath;
    }
    ...
    /**
      ...
      */
    public List<SqlitedbDatabaseHandler.AsyncTasks> getAsyncTasks() {
        if (mbtilesSplitter.getmbtiles() == null) { // in case .'open' was forgotten
            open(); // "" : default value will be used '1.1'
        }
    ...
    public List<SpatialRasterTable> getSpatialRasterTables( boolean forceRead ) throws Exception {
        if (rasterTableList == null || forceRead) {
            ...
            SpatialRasterTable table = new SpatialRasterTable(databasePath, databaseBean.getTableName(), "4326",
                    databaseBean.getMinScale(), databaseBean.getMaxScale(), databaseBean.getCenterLng(),
                    databaseBean.getCenterLat(), "?,?,?", d_bounds);
    ...
    public float[] getTableBounds( AbstractSpatialTable spatialTable ) throws Exception {
        SqlitedbMetadata metadata = mbtilesSplitter.getMetadata();
    ...
    */
    public void runRetrieveUrl( HashMap<String, String> mbtiles_request_url, HashMap<String, String> async_mbtiles_metadata ) {
        ...
        if (asyncTasksList.size() > 0) {
            mbtiles_async = new SqlitedbAsync(this);
